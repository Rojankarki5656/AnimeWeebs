...
export default function homeExtract(html) {
  ...
  $($spotlight).each((i, el) => {
    ...
    obj.episodes.sub = Number(
      details.find(".tick-item.tick-sub").text().trim()
    );
    obj.episodes.dub = Number(
      details.find(".tick-item.tick-dub").text().trim()
    );

    const epsText = details.find(".tick-item.tick-eps").length
      ? details.find(".tick-item.tick-eps").text().trim()
      : details.find(".tick-item.tick-sub").text().trim();
    ...
    const res = $topTen
      ...
      .map((i, el) => {
        const obj = {
          ...
          rank: i + 1,
          alternativeTitle:
            $(el).find(".film-name a").attr("data-jname") || null,
